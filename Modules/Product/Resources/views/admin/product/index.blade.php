<x-admin.layout.app>
    <x-slot name="header">
        ...
        <x-admin.layout.sections.page-button :href="route('admin.products.create')">
            Cadastrar novo
    ...
    <x-admin.layout.sections.list-section>
        <x-admin.layout.sections.filter gridCols="sm:grid-cols-2">
            ...
            <x-admin.layout.sections.filter-input inpName="name" title="Nome" placeholder="Nome do produto" />
            ...
            <x-slot name="tbody">
                @forelse ($collection as $item)
                    <tr class="bg-white border-b">
                        ...
                        <x-admin.layout.sections.table-td>
                            <x-admin.layout.sections.status-badge :condition="$item->type->venda()" :trueTitle="$item->type->texto()"
                                :falseTitle="$item->type->texto()" />
                            <br />
                            <x-admin.layout.sections.status-badge :condition="$item->has_child->nenhuma()" :trueTitle="$item->has_child->texto()"
                        ...
                        <x-admin.layout.sections.table-actions-td>
                            <x-admin.layout.sections.table-action :href="route('admin.products.edit', $item->id)" title="Editar">
                                <i class="text-base ti ti-edit"></i>
                            </x-admin.layout.sections.table-action>
                            <x-admin.layout.sections.table-action :href="route('admin.products.destroy', $item->id)" title="Excluir" :destroy="true">
