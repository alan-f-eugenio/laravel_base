<x-admin-layout>
    <x-slot name="header">
        ...
        <x-admin.page-button :href="route('admin.customers.index')">
            Listar Clientes
        </x-admin.page-button>
    </x-slot>
    <x-admin.list-section>
        <x-admin.form :action="$item->id ? route('admin.customers.update', $item->id) : route('admin.customers.store')" :editing="(bool) $item->id" x-data="{!! '{personFisica: ' . ($personFisica ? 'true' : 'false') . '}' !!}">
            <x-admin.form-grid gridCols="sm:grid-cols-3">
                <x-admin.form-subtitle>Dados de Identificação</x-admin.form-subtitle>
                ...
                <x-admin.form-label inpName="cpf" title="CPF">
                    <x-admin.form-input inpName="cpf" placeholder="000.000.000-00" :inpValue="old('cpf') ?: $item->cpf"
                        required />
                </x-admin.form-label>
                ...
                <x-admin.form-label inpName="rg" title="RG">
                    <x-admin.form-input inpName="rg" placeholder="Identidade" :inpValue="old('rg') ?: $item->rg" />
                </x-admin.form-label>
                ...
                <x-admin.form-label inpName="state_registration" title="Inscrição Estadual"
                    ...
                    <x-admin.form-input inpName="state_registration" placeholder="Inscrição Estadual"
            ...
            <x-admin.form-grid gridCols="sm:grid-cols-3">
                ...
                <x-admin.form-label inpName="neighborhood" title="Bairro">
                    <x-admin.form-input inpName="neighborhood" placeholder="Bairro" :inpValue="old('neighborhood') ?: $address->neighborhood"
                        required />
                </x-admin.form-label>
                ...
                <x-admin.form-subtitle>Dados de Contato e Acesso</x-admin.form-subtitle>
                <x-admin.form-label inpName="phone" title="Telefone">
                    <x-admin.form-input inpName="phone" placeholder="(00) 00000-0000" :inpValue="old('phone') ?: $item->phone"
                        required />
                </x-admin.form-label>
