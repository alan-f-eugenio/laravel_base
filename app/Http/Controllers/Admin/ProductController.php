...
use App\Http\Requests\Admin\ProductChildAdminRequest;
...
class ProductController extends Controller {
    ...
    public function index(Request $request) {
        ...
        return view('admin.products.index', ['collection' => $query->paginate(10)]);
    }
     ...
     */
    public function store(ProductAdminRequest $request, ProductChildAdminRequest $childRequest) {
        //
     ...
     */
    public function update(ProductAdminRequest $request, ProductChildAdminRequest $childRequest, Product $product) {
        //
        $attributes = $request->validated();
        $attributes['slug'] = Str::slug($attributes['name']);
        ...
        DB::transaction(function () use ($product, $attributes, $childAttributes, $listFiles, $listOldFiles, $listAllFiles) {
            ...
            // dd($listAllSecCats, $attributes['product_secondary_category_id'], request()->all());
            foreach ($product->secondaryCategories as $secCat) {
